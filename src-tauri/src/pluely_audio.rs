...
pub async fn start_capture_with_transcription(&mut self) -> Result<()> {
            ...
            if is_speech {
                ...
                if self.speech_buffer.len() > max_samples {
                    if let Ok(b64) = self.samples_to_wav_b64(&self.speech_buffer) {
                        ...
                        if let Err(e) = self.save_audio_debug_file(&self.speech_buffer, "safety_cap") {
                            error!("Failed to save debug audio file: {}", e);
                        }
                ...
                if self.in_speech {
                    ...
                    if self.silence_chunks >= SILENCE_CHUNKS {
                        if self.speech_chunks >= MIN_SPEECH_CHUNKS && !self.speech_buffer.is_empty() {
                            ...
                            if let Ok(b64) = self.samples_to_wav_b64(&self.speech_buffer) {
                                // Save to local file for debugging
                                if let Err(e) = self.save_audio_debug_file(&self.speech_buffer, "speech_segment") {
                                    error!("Failed to save debug audio file: {}", e);
                                }
                                let _ = self.app_handle.emit("speech-detected", b64);
    ...
    /// Save audio samples to local file for debugging
    fn save_audio_debug_file(&self, samples: &[f32], prefix: &str) -> Result<(), String> {
        use std::fs;
        
        // Create debug directory outside src-tauri to avoid triggering cargo watch restarts
        // This goes to desktopapp/audiodebug instead of src-tauri/audio_debug
        let debug_dir = "../audiodebug";
        ...
        let filename = format!("{}/{}_{}.wav", debug_dir, prefix, timestamp);
        ...
        info!("🎵 DEBUG: Saved audio file: {} ({} samples, {:.2}s)", 
              filename, samples.len(), samples.len() as f32 / self.sample_rate as f32);
